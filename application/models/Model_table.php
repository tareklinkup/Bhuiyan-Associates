...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select paid_purchase + paid_material_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset
            ) as total_out,
            (
    ...
    public function currentStock($clauses = '', $data = null)
    {
        if($data == null){
            $data = (object)['branchId'=>''];
        }
        $stock = $this->db->query("
            select * from(
                select
